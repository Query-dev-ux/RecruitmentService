...
- `hh_vacancy_id` — **отдельное поле**, ID вакансии на **hh.ru** (не путать с
  `crm_vacancy_id`). Если указан — шаблон дополнительно подтягивает входящие отклики на
  эту вакансию с HH (см. раздел 6). Опционален и независим от `criteria` — можно
  указать только его, без единого критерия поиска, и активный поиск по резюме тогда не
  запустится вообще, а отклики — будут.
...
- `vacancy_ref` — только для Telegram: на какую вакансию откликнулся (см. ниже)
...
- `vacancy_ref` — на какую вакансию откликнулся кандидат; заполняется только для
  Telegram-откликов, из того же `vacancy_ref`, что бот передаёт в
  `POST /telegram/applications` (см. раздел 4) — то есть это ровно та строка,
  которую прислал бот (у бота сейчас это название вакансии, например `"Media
  Buyer (Facebook)"`, а не `crm_vacancy_id`; см. предупреждение в разделе 4).
  `null` для HH-кандидатов — там вакансия определяется через
  `scores[].search_template_id` → `SearchTemplate.hh_vacancy_id`, это другая
  модель (кандидата могут оценивать сразу по нескольким шаблонам/вакансиям, а
  Telegram-отклик — всегда по одной конкретной). Если кандидат отвечает
  несколько раз, здесь остаётся вакансия последнего отклика *с указанной
  вакансией* — отклик без вакансии (например через «Отправить резюме») не
  затирает то, что было известно раньше.
...
POST /telegram/applications
{
  ...
  "telegram_full_name": "Daniil CG",   // из message.from_user.full_name
  "telegram_username": "daniil_cg",     // опционально — становится sources[].external_url = t.me/<username>
...
**Уже подключено и работает в проде** — существующий Telegram-бот (CGBot) отправляет
сюда каждый отклик (в дополнение к пересылке в HR-группу Telegram, которая осталась
как есть).

**На какую вакансию откликнулся кандидат** — это то, что бот передаёт в
`vacancy_ref`, и теперь оно возвращается на самом кандидате как
`ExternalCandidateOut.vacancy_ref` (см. раздел 3) — до этого поле сохранялось у
нас, но нигде не отдавалось через API, что и было тем самым пробелом.
Важно: сейчас бот шлёт туда **название вакансии** (например `"Media Buyer
(Facebook)"`), а не какой-то отдельный ID — у бота список вакансий захардкожен
и общего с CRM идентификатора пока нет. Значит для скоринга по шаблону
(`scored_against_templates`) `crm_vacancy_id` шаблона должен быть заведён как
ровно эта строка. Если у вакансии в CRM другой ID/слаг — либо заводите
`crm_vacancy_id` под название бота, либо это отдельная задача на синхронизацию
идентификаторов между ботом и CRM (сейчас не сделано).
...
## 5. HH-отклики на размещённые вакансии

**Реализовано.** Это отдельный от активного поиска резюме источник — входящие отклики
на вакансию, которую вы разместили на hh.ru, а не результаты поиска по базе. Не
требует платного тарифа «доступ к базе резюме».

Отдельного эндпоинта для этого нет — механизм встроен в уже знакомый `search_runs`:
если у шаблона указан `hh_vacancy_id`, каждый его запуск (ручной или по расписанию)
...
категория откликов (`response` в терминах HH) — уже приглашённые/отклонённые кандидаты
через этот механизм не подтягиваются повторно, это дальше зона ответственности CRM.

---

## 6. HH — статус подключения
...
- Форма шаблона:
  ...
  - Опционально — ID вакансии на hh.ru (→ `hh_vacancy_id`), если вакансия также
    размещена на HH и нужно подтягивать входящие отклики на неё (см. раздел 5)
...
- Карточка: имя (у HH — только после раскрытия контактов, но во входящих откликах имя
  ...
  клику — разбивка score (`breakdown`).
