...
Можно не делать отдельным экраном — просто фильтр `source=telegram` на экране
«Кандидаты». Актуально только после того, как CGBot подключат к
`/telegram/applications` (сейчас не подключён).
